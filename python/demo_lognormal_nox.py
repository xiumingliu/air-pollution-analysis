...
# Pre-processing
# =============================================================================
nox = np.array(nox).reshape(5, -1)

# Replace zeros with average
nox = functions.replace_zero_with_average(nox)

# Log-normal transform
nox_transformed = functions.log_normal(nox)
...
data_test = nox_transformed[:, index_test]
data_train = nox_transformed[:, index_train]
...
#    axs[i].plot(samples_median[i, :], 'k') 
    ...
    axs[i].set_ylim([0, .8*np.max(nox)])
axs[0].set_ylabel(r'NO$_{x}$ ($\mu g/m^3$)')
