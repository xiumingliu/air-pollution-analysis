...
# Configuration
# =============================================================================
test_day_start = np.datetime64('2014-01-01', dtype='datetime64[D]')
test_day_end = np.datetime64('2014-01-03', dtype='datetime64[D]')
...
for day in np.arange(test_day_start, test_day_end, dtype='datetime64[D]'):
    ...
    time_train = np.arange('2006-01-01', day, dtype='datetime64[h]')
...
for i in range(5):
    plt.plot(mse[i, :], label = 'Station %i' % i)
plt.xlim([0, 23])
plt.ylim([0, 1000])
plt.legend()
