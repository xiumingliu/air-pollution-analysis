...
feature_train_holiday = np.transpose(feature_train_holiday.reshape(6, -1, 24), axes=(1, 0, 2))
data_train_holiday = np.transpose(data_train_holiday.reshape(5, -1, 24), axes=(1, 0, 2))

x_train = np.empty((feature_train_holiday.shape[0], 6*24))
y_train = np.empty((data_train_holiday.shape[0], 5*24))
for i in range(feature_train_holiday.shape[0]):
    x_train[i, :] = np.concatenate(feature_train_holiday[i, :, :])
    y_train[i, :] = np.concatenate(data_train_holiday[i, :, :])
...
mc = ModelCheckpoint('bestmodel_pm10_5l_holiday.h5', monitor='val_loss', mode='min', verbose=1, save_best_only=True)

NN_model.fit(x_train, y_train, epochs=500, batch_size=10, validation_split = 0.2, callbacks=[es, mc])

#NN_model.save("model_pm10_5l_busday.h5")

NN_model = load_model("bestmodel_pm10_5l_holiday.h5")
